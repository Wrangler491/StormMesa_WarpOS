...
void asm_shade_rgba_fast(GLuint n, GLuint side, GLfloat normal[][3],
			 GLubyte color[][4], GLfloat basecolor[4],
			 shade_opt *sh);
#endif
	 ...
	 if (n_dot_VP<=0.0F) {
	    ...
	    if (ctx->Light.Model.LocalViewer) {
		  ...
		  vz *= invlen;
	       ...
	       if (vlen >0.000001F)
	       {
		  GLfloat invlen = fast_invsqrt(vlen);
		  vx *= invlen;
		  vy *= invlen;
		  vz *= invlen;
	       }
...
#else
	       n_dot_h = n_dot_h * fast_invsqrt( h_x*h_x + h_y*h_y + h_z*h_z );
#endif
	       if (n_dot_h>1.0F) {
...
#else
		     mat->ShineTable[k] = pow( n_dot_h, mat->Shininess );
#endif
		  }
...
#else
	    d = VPx*VPx + VPy*VPy + VPz*VPz;
	    if (d >0.01F)
	    {
	       GLfloat invd = fast_invsqrt(d);
	       VPx *= invd;
	       VPy *= invd;
	       VPz *= invd;
	    }
...
#if defined(AMIGA) && defined(__PPC__) && !defined(WARPUP)
   {
	struct gl_light* light = ctx->Light.FirstEnabled;
	if (!light)
	{
		asm_shade_rgba_fast(n,side,normal,color,baseColor,NULL);
		return;
	}
	if (light && !light->NextEnabled)
	{
		shade_opt sh;

		sh.VP_inf_norm = light->VP_inf_norm;
		sh.h_inf_norm = light->h_inf_norm;
		sh.MatDiffuse = light->MatDiffuse[side];
		sh.MatSpecular = light->MatSpecular[side];
		sh.Shininess = ctx->Light.Material[side].Shininess;
		sh.ShineTable = ctx->Light.Material[side].ShineTable;
		asm_shade_rgba_fast(n,side,normal,color,baseColor,&sh);
		return;
	}
   }
#endif
