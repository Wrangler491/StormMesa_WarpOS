...
#ifdef WARPUP
   extern GLcontext *CC; 	 				/* All threads use same pointer to current context    */
#else

#ifdef THREADS
   extern GLcontext *gl_get_thread_context( void );	/* A seperate GLcontext for each thread */
#else

#ifdef AMIGA
   extern GLcontext *gl_get_task_context(void );	/* A seperate GLcontext for each Library Base */
#else
   extern GLcontext *CC; 	 				/* All threads use same pointer to current context    */
#endif
