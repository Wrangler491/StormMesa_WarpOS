...
#else
      ndc_x = VB->Clip[v1][0] / VB->Clip[v1][3];
      ndc_y = VB->Clip[v1][1] / VB->Clip[v1][3];
#endif
...
#else
      ndc_x = VB->Clip[v2][0] / VB->Clip[v2][3];
      ndc_y = VB->Clip[v2][1] / VB->Clip[v2][3];
#endif
...
static void render_polygon( GLcontext *ctx, GLuint n, GLuint vlist[] )
{
   ...
   /* Compute orientation of polygon, do cull test, offset, etc */
   {
      ...
      if (ctx->Polygon.Unfilled) {
        ...
        if (ctx->Driver.TriFanFunc && ok)
        {
                (*ctx->Driver.TriFanFunc)( ctx, j0, vlist[1], vlist[n-1]);
        }
        else
        {
#endif
...
#else
            win[i][0] = (VB->Clip[i][0] /VB->Clip[i][3]) * sx + tx;
            win[i][1] = (VB->Clip[i][1] /VB->Clip[i][3]) * sy + ty;
#endif
      ...
      case GL_LINE_STRIP:
         ...
         if (VB->ClipOrMask) {
...
#endif
#ifdef DRIVEREXT
                     if (ctx->Driver.LineStripFunc && ok)
                     {
                        if (start < 0)
                        {
                                start = i;
                        ...
                        {
                                (*ctx->Driver.LineStripFunc)( ctx, start-1, end-1);
                                start = i;
                        }
                        end = i+1;
                     }
...
#ifdef DRIVEREXT
            if ((ctx->Driver.LineStripFunc) && ok && (start >= 0))
            {
                (*ctx->Driver.LineStripFunc)( ctx, start-1, end-1);
            }
#endif
            ...
            if ((ctx->Light.ShadeModel == GL_FLAT) &&
              ...
              if (ctx->Driver.LineStripFunc && ok)
              {
                (*ctx->Driver.LineStripFunc)( ctx, 0, VB->Count-1);
              }
              else
              {
#endif
...
/* UseHW_DirectTriangles is like ctx->DirectTriangles but without culling test. As culling is maded in hardware in HW_DrawTriangles() */
int UseHW_DirectTriangles;

		if (	ctx->Polygon.Unfilled
		|| ctx->Polygon.OffsetAny
		|| ctx->Light.Model.TwoSide
		|| ctx->RenderMode!=GL_RENDER) 
			{ UseHW_DirectTriangles = GL_FALSE; }
		else
			{ UseHW_DirectTriangles = GL_TRUE;  }

/* Check if it use  Thelllier's patch ? If yes can draw fastly with w3d_DrawArrray() */
		if(!StormMesa.UseDrawArray.ON) UseHW_DirectTriangles = GL_FALSE;

            if (UseHW_DirectTriangles) 
		{
                  START_PROFILE
			HW_DrawTriangles(ctx,VB->Count);
                  END_PROFILE( ctx->PolygonTime, ctx->PolygonCount,VB->Count/3)
      	}
            else 
		{
			GLuint i;
      ...
      case GL_TRIANGLE_STRIP:
         ...
         if (VB->ClipOrMask) {
               ...
               if (VB->ClipMask[i-2] & VB->ClipMask[i-1]
                  ...
                  if (ctx->DirectTriangles) {
                     ...
                     if (ctx->Driver.TriStripFunc && ok)
                     {
                        ...
                        {
                                (*ctx->Driver.TriStripFunc)( ctx, start-2, end-1);
                                start = i;
                        }
                        end = i+1;
                     }
...
#ifdef DRIVEREXT
            if ((ctx->Driver.TriStripFunc) && ok && (start >= 0))
            {
                (*ctx->Driver.TriStripFunc)( ctx, start-2, end-1);
            }
#endif
            ...
            if ((ctx->Light.ShadeModel == GL_FLAT) &&
              ...
              if (ctx->Driver.TriStripFunc && ok)
              {
                (*ctx->Driver.TriStripFunc)( ctx, 0, VB->Count-1);
              }
              else
              {
#endif
      ...
      case GL_TRIANGLE_FAN:
         ...
         if (VB->ClipOrMask) {
               ...
               if (VB->ClipMask[0] & VB->ClipMask[i-1] & VB->ClipMask[i]
                  ...
                  if (ctx->DirectTriangles) {
                     ...
                     if (ctx->Driver.TriFanFunc && ok)
                     {
                        ...
                        {
                                (*ctx->Driver.TriFanFunc)( ctx, 0, start-1, end-1);
                                start = i;
                        }
                        end = i+1;
                     }
...
#ifdef DRIVEREXT
            if ((ctx->Driver.TriFanFunc) && ok && (start >= 0))
            {
                (*ctx->Driver.TriFanFunc)( ctx, 0, start-1, end-1);
            }
#endif
            ...
            if ((ctx->Light.ShadeModel == GL_FLAT) &&
              ...
              if (ctx->Driver.TriFanFunc && ok)
              {
                (*ctx->Driver.TriFanFunc)( ctx, 0, 1, VB->Count-1);
              }
              else
              {
#endif
      ...
      case GL_QUAD_STRIP:
         ...
         if (VB->ClipOrMask) {
               ...
               if (VB->ClipMask[i-2] & VB->ClipMask[i-3]
                  ...
                  if (ctx->DirectTriangles) {
                     ...
                     if (ctx->Driver.TriStripFunc && ok)
                     {
                        ...
                        {
                                (*ctx->Driver.TriStripFunc)( ctx, start-3, end-2);
                                start = i;
                        }
                        end = i+2;
                     }
            ...
            if ((ctx->Light.ShadeModel == GL_FLAT) &&
              ...
              if (ctx->Driver.TriStripFunc && ok)
              {
                (*ctx->Driver.TriStripFunc)( ctx, 0, ((VB->Count)&0xfffffffe)-1);
              }
              else
              {
#endif
