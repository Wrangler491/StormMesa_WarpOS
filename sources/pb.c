...
#else
	    if (gl_alpha_test( ctx, PB->count, (CONST GLubyte(*)[4])PB->rgba, mask )==0) {
#endif
	       goto CleanUp;
	    }
	 }
...
REM(regular depth testing RGBA COLOR PIXELS)
...
#else
	    gl_write_alpha_pixels( ctx, PB->count, PB->x, PB->y, (CONST GLubyte(*)[4])PB->rgba, mask );
#endif
	 }
	 ...
	 if (ctx->Stencil.Enabled) {
	    ...
	    /* regular depth testing */
REM(regular depth testing : Same color for all pixels )
	    (*ctx->Driver.DepthTestPixels)( ctx, PB->count, PB->x, PB->y, PB->z, mask );
	 }
	 ...
	 if (ctx->Stencil.Enabled) {
	    ...
	    /* regular depth testing */
REM(regular depth testing COLOR INDEX PIXELS)
	    (*ctx->Driver.DepthTestPixels)( ctx, PB->count, PB->x, PB->y, PB->z, mask );
	 }
	 ...
	 if (ctx->Stencil.Enabled) {
	    ...
	    /* regular depth testing */
REM(regular depth testing: Same color index for all pixels)
	    (*ctx->Driver.DepthTestPixels)( ctx, PB->count, PB->x, PB->y, PB->z, mask );
	 }
