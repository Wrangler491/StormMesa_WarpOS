...
#if defined(AMIGA) && defined(__PPC__)  && !defined(WARPUP)
void asm_vp_map_vertices_now(const GLubyte clipMask[],int n,GLfloat vWin[][3],const GLfloat vClip[][4],
			     GLfloat sx, GLfloat sy, GLfloat sz,
			     GLfloat tx, GLfloat ty, GLfloat tz);
void asm_vp_map_vertices(const GLubyte clipMask[],int n,GLfloat vWin[][3],const GLfloat vClip[][4],
			 GLfloat sx, GLfloat sy, GLfloat sz,
			 GLfloat tx, GLfloat ty, GLfloat tz);
#endif
...
#define END_PROFILE( TIMER) TIMER += (gl_time() - t0);
  ...
  GLdouble t0,t1;
  GLdouble prof1_time,prof2_time,prof3_time,prof4_time,prof5_time,prof6_time,prof7_time,prof8_time,profrender_time;
#else
...
#ifdef AMIGA
#define START_PROFILE3
#define END_PROFILE3( TIMER)
#endif
...
#else
      case MATRIX_GENERAL:
	 asm_transform_points4_general( n, t, ctx->TextureMatrix[texSet], (CONST GLfloat(*)[4])t );
	 break;
      case MATRIX_IDENTITY:
	 /* Do nothing */
	 break;
      case MATRIX_2D:
	 asm_transform_points4_2d( n, t, ctx->TextureMatrix[texSet], (CONST GLfloat(*)[4])t );
	 break;
      case MATRIX_3D:
	 asm_transform_points4_3d( n, t, ctx->TextureMatrix[texSet], (CONST GLfloat(*)[4])t );
	 break;
#endif
      default:
	 /* should never get here */
...
#else
      asm_vp_map_vertices_now(clipMask,n,vWin,vClip,sx,sy,sz,tx,ty,tz);
#endif
   }
   else {
#if !defined(AMIGA) || !defined(__PPC__) || defined(WARPUP)
...
#else
      asm_vp_map_vertices(clipMask,n,vWin,vClip,sx,sy,sz,tx,ty,tz);
#endif
   }
				   ...
				   GLboolean firstPartToDo)
{
...
#else
  project_and_cliptest( ctx, vbCount - vbStart, (CONST GLfloat(*)[4])(VB->Eye + vbStart),
			VB->Clip + vbStart, VB->ClipMask + vbStart,
			clipOrMask, clipAndMask );
#endif
  if (*clipAndMask) {
  ...
  if (ctx->DoViewportMapping) {
     ...
     viewport_map_vertices( ctx, vbCount - vbStart, VB->Clip + vbStart,
		     ...
		     VB->Win + vbStart );
#else
     viewport_map_vertices( ctx, vbCount - vbStart, (CONST GLfloat(*)[4])(VB->Clip + vbStart),
		     (*clipOrMask) ? VB->ClipMask + vbStart : (GLubyte*) NULL,
		     VB->Win + vbStart );
#endif
  }
    END_PROFILE3(prof7_time)
    START_PROFILE3
